...
// ── p₀* Korrektur bei fehlendem Linksabbiegestreifen auf HS [F22] ─────────────
// qj = Geradeausfahrer [PWE/h], qk = Rechtsabbieger [PWE/h] auf gleicher Zufahrt
function p0Star(q: number, L: number, qj: number, qk: number): number {
  if (L <= 0) return 0
  const denom = 1800 - qj - qk
  if (denom <= 0) return 0
  return Math.max(0, 1 - (q / L) * (1800 / denom))
}
...
// ── Wartezeit [F25] — Abb. 4, SN 640 022 ─────────────────────────────────────
// Die Norm gibt w nur grafisch an (Abb. 4, «nach Kimber, Hollis, 1979»).
// Algebraische Form: Wartezeitformel des HBS 2015 (= Bedienzeit 3600/L +
// Stau-Term nach Akçelik/Troutbeck 1991; Herleitung: Brilon 2008, TRR 2071,
// Gl. 9, Fall D2+A2; vgl. Brilon-Vortrag 2016, Folie «Wartezeitformel»).
// w = 3600/L + 900·T · [(a−1) + √((a−1)² + (3600/L · a) / (450·T))]
// T = 1.0 h (Spitzenstunde) reproduziert die Abb.-4-Kurven — verifiziert an
// 14 abgelesenen Stützpunkten (Juni 2026, siehe Test «Abb. 4 — Wartezeitkurven»).
// Gibt auch bei q=0 (a=0) die Bedienzeit 3600/L zurück — nie 0.
export function w(q: number, L: number): number {
  if (L <= 0) return 0
  const a   = q / L
...
function einmuendung(v: number[][], raw: number[][], flags: SN640022LaneFlags): SN640022Result {
  ...
  // Rang 3 — p0,7 ggf. durch p0,7* ersetzen [F22]
  const q8_pwe = v[1][0], q9_pwe = v[1][3] ?? 0
  const p07 = (flags.armCLeftLane ?? true)
    ? p0(q7, L7)
    : p0Star(q7, L7, q8_pwe, q9_pwe)
...
function kreuzung(v: number[][], raw: number[][], flags: SN640022LaneFlags): SN640022Result {
  ...
  // Rang 3 — p0,1 / p0,7 ggf. durch p0* ersetzen [F22]
  const q2_pwe = v[0][1], q3_pwe = v[0][2]
  const q8_pwe = v[1][0], q9_pwe = v[1][3]
  const p01 = (flags.armALeftLane ?? true)
    ? p0(q1, L1)
    : p0Star(q1, L1, q2_pwe, q3_pwe)
  const p07 = (flags.armCLeftLane ?? true)
    ? p0(q7, L7)
    : p0Star(q7, L7, q8_pwe, q9_pwe)
  const px = p01 * p07

...
// ── Tabelle 2 ─────────────────────────────────────────────────────────────────
export interface Tab2Entry { tGrSum: number; lambdaSum: number; qKritMax: number }
export interface Tab2Row   { Z: number; p2: Tab2Entry; p3: Tab2Entry; p4: Tab2Entry }

export const TAB2: Tab2Row[] = [
...
const PHASE_PLAN_LABELS: Record<string, string[]> = {
...
// ── Wartezeit (VSS 40 023a, Ziffer 12; nach Kimber & Hollis 1979, TRRL LR 909) ─
// w_m = w_1 + w_0
// w_1 = Z·(1−λ)² / (2·(1−λ·X))
// w_0 = 900·[(X−1) − 2C·X/Q + √((X−1)² + 4C·(X+1+C·X/Q) / (Q/X))]
// C = 0.5 für isolierte LSA
export function streamDelay(
  Q: number, lambda: number, Z: number, sSat: number = S,
